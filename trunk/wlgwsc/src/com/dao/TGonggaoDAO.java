...
public class TGonggaoDAO extends HibernateDaoSupport
{
	...
	public List find(String gonggaoTitle)
	{
		log.debug("finding  TGonggao instances");
		try
		{
			String queryString = "from TGonggao where gonggaoTitle like '%"+gonggaoTitle+"%' order by gonggaoId desc";
			return getHibernateTemplate().find(queryString);
		} catch (RuntimeException re)
		{
